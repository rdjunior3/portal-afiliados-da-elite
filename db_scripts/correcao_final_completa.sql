-- ================================================================
-- CORREÇÃO FINAL COMPLETA - PORTAL AFILIADOS DA ELITE
-- Versão robusta que remove TODAS as políticas antes de recriar
...
BEGIN
    ...
    -- Remover TODAS as políticas do storage.objects relacionadas a product-images
    ...
    LOOP
        ...
        RAISE NOTICE 'Removida política storage: %', policy_record.policyname;
    END LOOP;
    
    -- Remover TODAS as políticas da tabela products
    FOR policy_record IN 
        SELECT policyname FROM pg_policies 
        WHERE schemaname = 'public' AND tablename = 'products'
    LOOP
        EXECUTE 'DROP POLICY IF EXISTS "' || policy_record.policyname || '" ON products';
        RAISE NOTICE 'Removida política products: %', policy_record.policyname;
    END LOOP;
    
    -- Remover TODAS as políticas da tabela product_offers
    FOR policy_record IN 
        SELECT policyname FROM pg_policies 
        WHERE schemaname = 'public' AND tablename = 'product_offers'
    LOOP
        EXECUTE 'DROP POLICY IF EXISTS "' || policy_record.policyname || '" ON product_offers';
        RAISE NOTICE 'Removida política product_offers: %', policy_record.policyname;
    END LOOP;
